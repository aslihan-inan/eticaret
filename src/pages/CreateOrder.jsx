...
export default function CheckoutPage() {
  ...
  const [newCard, setNewCard] = useState({ card_no:"", expire_month:"", expire_year:"", name_on_card:"", ccv:"" });
    ...
    const fetchAll = async () => {
      try {
        const [cardsRes, addrRes] = await Promise.all([
          api.get("/user/card"),       // backend endpoint
  ...
  const handleSaveAddress = async () => {
    ...
    if (!title || !name || !surname || !phone || !city) return toast.error("Tüm zorunlu alanları doldurun!");
    try {
  ...
  const resetCardForm = () => {
    setNewCard({ card_no:"", expire_month:"", expire_year:"", name_on_card:"", ccv:"" });
  ...
  // ---------------- HESAPLAMA ----------------
  const productTotal = cartItems.reduce((acc, item) => acc + (item.product.price || 0) * item.count, 0);
  ...
  const handleCheckout = async () => {
    ...
    const payload = {
      ...
      price: total,
      products: cartItems.map(i => ({ product_id:i.product.id, count:i.count, detail:i.product.detail || "" }))
    };
    ...
    <div className="max-w-6xl mx-auto py-8 px-4 grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="border rounded-2xl p-4 shadow-sm">
          ...
          <button onClick={()=>setShowAddressForm(true)} className="flex items-center justify-center border-2 border-dashed rounded-xl p-6 text-gray-500 hover:bg-gray-50 w-full">
          ...
          <div className="grid md:grid-cols-2 gap-4 mt-4">
            {addresses.map(addr => (
              <div key={addr.id} onClick={()=>setSelectedAddressId(addr.id)} 
                   ...
                   ${selectedAddressId===addr.id ? "border-orange-500 bg-orange-50 shadow" : "border-gray-200 hover:bg-gray-50"}`}>
        ...
        <div className="border rounded-2xl p-4 shadow-sm space-y-4">
          ...
          <div className="space-y-2">
            {cards.map(card => (
              <div key={card.id} onClick={()=>setSelectedCardId(card.id)} 
                   ...
                   ${selectedCardId===card.id ? "border-orange-500 bg-orange-50 shadow" : "border-gray-200 hover:bg-gray-50"}`}>
                <div>
          ...
          {!showCardForm && <button onClick={()=>setShowCardForm(true)} className="text-sm text-blue-600 hover:underline mt-2">+ Yeni Kart Ekle</button>}

          {showCardForm && (
            <form onSubmit={isEditingCard ? handleUpdateCard : handleAddCard} className="space-y-3 mt-2">
              <input name="card_no" type="text" placeholder="Kart Numarası" value={newCard.card_no} onChange={handleCardChange} className="w-full border p-2 rounded-lg" required/>
              <div className="flex space-x-2">
                <input name="expire_month" type="number" placeholder="Ay" value={newCard.expire_month} onChange={handleCardChange} className="w-1/2 border p-2 rounded-lg" required/>
                <input name="expire_year" type="number" placeholder="Yıl" value={newCard.expire_year} onChange={handleCardChange} className="w-1/2 border p-2 rounded-lg" required/>
              </div>
              <input name="name_on_card" type="text" placeholder="Kart Üzerindeki İsim" value={newCard.name_on_card} onChange={handleCardChange} className="w-full border p-2 rounded-lg" required/>
              <input name="ccv" type="number" placeholder="CCV" value={newCard.ccv} onChange={handleCardChange} className="w-full border p-2 rounded-lg" required/>
              <div className="flex flex-col sm:flex-row gap-2">
