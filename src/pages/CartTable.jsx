...
import { addToCart, increaseCount, decreaseCount, toggleChecked, removeFromCart } from "../redux/slices/cartSlice";
...
export default function CartTable() {
  ...
  const handleExternalRemove = (productId) => {
    setExternalProducts(prev => {
      const updatedProducts = {...prev};
      updatedProducts[activeTab] = updatedProducts[activeTab].filter(product => product.id !== productId);
      return updatedProducts;
    });
  };
  ...
  return (
    <div className="max-w-6xl mx-auto mt-10 p-4">
          ...
          <tbody>
            {cart.map(item => (
              <tr key={item.product.id} className="border-b">
                ...
                <td className="p-2 text-center">{item.product.price} TL</td>
                <td className="p-2 text-center font-medium">{item.count}</td>
                <td className="p-2 text-center font-medium">{item.count * item.product.price} TL</td>
      ...
      <div className="flex mb-4 border-b">
        <button
          className={`py-3 px-6 font-medium text-lg ${activeTab === "previouslyAdded" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600"}`}
          onClick={() => setActiveTab("previouslyAdded")}
        >
          Önceden Eklediklerim
        </button>
        <button
          className={`py-3 px-6 font-medium text-lg ${activeTab === "recommended" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600"}`}
          onClick={() => setActiveTab("recommended")}
        >
          Önerilen Ürünler
        </button>
        <button
          className={`py-3 px-6 font-medium text-lg ${activeTab === "favorites" ? "text-blue-600 border-b-2 border-blue-600" : "text-gray-600"}`}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    {/* Butonlar ürün isminin altına eklendi */}
                    <div className="flex items-center space-x-2 mt-2">
                      <button 
                        className="px-2 py-1 bg-gray-200 rounded hover:bg-gray-300 text-sm"
